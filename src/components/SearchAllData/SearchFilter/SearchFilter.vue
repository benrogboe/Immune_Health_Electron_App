...
export default {
  ...
  methods: {
    ...
    propertyHasUnit: function(propertyType) {
      ...
      return this.getUnitDisplayName(pathOr(unit, ['items', 'unit'], propertyType))
    },
    /**
    * Grabs the unit name and returns the displayName
    * @param {String} unit
    */
    getUnitDisplayName (unitName) {
      for (const dimension of this.scientificUnits) {
    ...
    setTargetFilter: function(item, type) {
      ...
      const filter = mergeRight(this.filter, {
        type,
        target: id,
        targetIntId: intId,
        targetLabel,
